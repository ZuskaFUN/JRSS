...
if (config.enableCluster && cluster.isMaster && process.env.NODE_ENV !== 'test' && process.env.NODE_ENV !== 'dev') {
    ...
    if (config.connect.socket) {
        ...
        server = app.listen(config.connect.socket, parseInt(config.listenInaddrAny) ? null : 'https://https://jrssfeeder.herokuapp.com');
    ...
    if (config.connect.port) {
        server = app.listen(config.connect.port, parseInt(config.listenInaddrAny) ? null : 'https://https://jrssfeeder.herokuapp.com');
