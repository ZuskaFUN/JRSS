        ...
        item:
            ...
            list
                .map((_, e) => {
                    ...
                    const title_path = e.find('span.title a');
                    const title_text = title_path.text();
                    const item_link = mainweb + title_path.attr('href');
                    const img_src = e.find('img').attr('src');
                    const vid_url = mainweb + `/embed/` + item_link.substring(item_link.lastIndexOf('=') + 1);
