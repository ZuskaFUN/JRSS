...
module.exports = async (ctx) => {
    ...
    const all = await Promise.all(
        types.map(async (type) => {
            const response = await got({
                method: 'get',
                url: `https://jrssfeeder.herokuapp.com/${lang}${type}.html`,
            });
    ...
    ctx.state.data = {
        ...
        link: 'https://https://jrssfeeder.herokuapp.com',
        ...
        item: list.map(({ page, item, type }) => {
            ...
            return {
                ...
                link: `https://https://jrssfeeder.herokuapp.com/${lang}${type}.html#${encodeURIComponent(titleEle.find('.header-anchor').attr('href') && titleEle.find('.header-anchor').attr('href').slice(1))}`,
                guid: item.attr('id'),
            };
        }),
    };
